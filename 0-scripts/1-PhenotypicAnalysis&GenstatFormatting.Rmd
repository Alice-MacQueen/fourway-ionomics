---
title: "1-Phenotypic Analysis and Genstat Formatting"
author: c("Li Zhang", "Alice MacQueen")
date: 2020-11-23
output: html_document
---

Troubleshooting table creation for Li's script #1.
...
```{r parts zero to three}

###0. set working directory, should have genotyping file and phenotyping file
workingdir <- file.path("../0-data")

###1. read the genotype .qua file to get the genotype ID
genotype = read.loci(file.path(workingdir, '4way.qua'),skip = 4)
genotype = genotype %>% dplyr::select(id) %>% dplyr::rename(LINE=id)

###2. read the raw phenotype file and Plant ID
phenotype_raw = read.csv(file.path(workingdir, 'Ionome.csv'))
plantlist = readxl::read_excel(file.path(workingdir, 
                                         'NSF_4WCR_Master Plant List_Final.xlsx'))

###3: get the phenotypes for the grandparents
parents = phenotype_raw %>% 
  left_join(plantlist, by=c('sample'='PLOT_GL')) %>%
  dplyr::select(SITE, LINE,B11:Cd111) %>%    # edited this for element names
  ...
  mutate(ECOTYPE = ifelse(LINE %in% c("WBC", "AP13"), "Lowland", "Upland"))
```
## Grandparents
```{r part four grandparent comparisons}
...
for (i in 3:20){
  ...
  for (s in unique(tmp3$SITE)){
    tmp4 = tmp %>% filter(SITE==s)
    if (i==15|i==16){tmp4 = tmp4 %>% filter(LINE!='VS16')}
    colnames(tmp4)[3] = "ion"
...
parents_comparison_ecotype = NULL
for (i in 3:20){
  print(i)
  tmp = parents[,c(1,2,i, 21)]
  tmp2 = tmp %>% group_by(SITE, ECOTYPE) %>% 
    get_summary_stats(colnames(parents)[i], type = "mean_se")
  tmp3 = tmp2 %>% unite(X, c('mean','se'), sep = "±") %>% select(-n)%>% spread(ECOTYPE,X)
  ...
  for (s in unique(tmp3$SITE)){
    ...
    if (i==15|i==16){tmp4 = tmp4 %>% filter(LINE!='VS16')}
    colnames(tmp4)[3] = "ion"
    res.aov <- tmp4 %>% anova_test(ion ~ ECOTYPE)###anova test, assuming equal variance
    res.oneway = oneway.test(tmp4$ion~tmp4$ECOTYPE, na.action = na.omit ) ##one way test,not necessarity equal variance
    p_aov = res.aov$p
    p_one = res.oneway$p.value 
    tmp5 = c(s, p_aov, p_one)
    tmp6 = rbind(tmp6, tmp5)
  ...
  tmp7 = tmp3 %>% left_join(tmp6 %>% as.data.frame())
  parents_comparison_ecotype = rbind(parents_comparison_ecotype,tmp7)
}

parents_comparison_lowland = NULL
for (i in 3:20){
  print(i)
  tmp = parents[,c(1,2,i, 21)]
  tmp2 = tmp %>% 
    filter(ECOTYPE == "Lowland") %>%
    group_by(SITE, LINE) %>% 
    get_summary_stats(colnames(parents)[i], type = "mean_se")
  tmp3 = tmp2 %>% unite(X, c('mean','se'), sep = "±") %>% select(-n)%>% spread(LINE,X)
  ...
  for (s in unique(tmp3$SITE)){
    if (i==15|i==16){next}
    tmp4 = tmp %>% filter(SITE==s & ECOTYPE == "Lowland")
    colnames(tmp4)[3] = "ion"
    res.aov <- tmp4 %>% anova_test(ion ~ LINE)###anova test, assuming equal variance
    res.oneway = oneway.test(tmp4$ion~tmp4$LINE, na.action = na.omit ) ##one way test,not necessarity equal variance
    p_aov = res.aov$p
  ...
  if (i==15|i==16){next}
  tmp7 = tmp3 %>% left_join(tmp6 %>% as.data.frame())
  parents_comparison_lowland = rbind(parents_comparison_lowland,tmp7)
  
}

parents_comparison_upland = NULL
for (i in 3:20){
  print(i)
  tmp = parents[,c(1,2,i, 21)]
  tmp2 = tmp %>% 
  ...
  for (s in unique(tmp3$SITE)){
    if (i==15|i==16){next}
    tmp4 = tmp %>% filter(SITE==s & ECOTYPE == "Upland")
    colnames(tmp4)[3] = "ion"
    res.aov <- tmp4 %>% anova_test(ion ~ LINE)###anova test, assuming equal variance
    res.oneway = oneway.test(tmp4$ion~tmp4$LINE, na.action = na.omit ) ##one way test,not necessarity equal variance
    p_aov = res.aov$p
  ...
  if (i==15|i==16){next}
  tmp7 = tmp3 %>% left_join(tmp6 %>% as.data.frame())
  parents_comparison_upland = rbind(parents_comparison_upland,tmp7)
}


parents_comparison %>% filter(as.numeric(`P-oneway`) < 0.05/54) %>% group_by(variable) %>% tally()
parents_comparison_upland %>% filter(as.numeric(`P-oneway`) < 0.05/54)
parents_comparison_lowland %>% filter(as.numeric(`P-oneway`) < 0.05/54)
parents_comparison_ecotype %>% filter(as.numeric(`P-oneway`) < 0.05/54)
...
## Generate cleaned phenotypes for Genstat
```{r part five formatting for genstat}
...
```{r part six garden by element welch one way tests}
...
```{r part 8 phenotypic correlations and rearrange elements}
...
```{r part nine format for Genstat}
...
p2 = ggplot(h2,aes(SITE,h2))+geom_col(alpha=0.6)+ facet_wrap(~Type+TRAIT, nrow=6,scales = 'free')+
  ...
  theme(aspect.ratio = 0.5/1)
3:41
F1 = plot_grid(p1,p2, labels = c('(a)','(b)'),ncol = 2, label_x=c(0.1,0.1))
save_plot(file='../1-PhenotypicAnalysis&GenstatFormatting/Fig1_PhenotypicVariation&heritability.tiff',
          F1, base_height = 8) (edited) 
```
